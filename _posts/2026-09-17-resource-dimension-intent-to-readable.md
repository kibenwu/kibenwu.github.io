...
> 素材来源：MetaProject 两个月开发过程 · `DesignDocument/` 设计文档 · `AGENTS.md`
> 与维度一的关系：维度一讲「意图 → 判据」，本章讲**判据如何变成可被眼睛读到的东西**。同一条脊，换一个通道。
> 状态标记：`[已有素材]` 工程里能取证 · `[待补]` 需要案例 · `[待议]` 讲法未定
>
> **本章定位：观点章 + 策展式取证。** 本项目美术资源全部来自商城、零自研，因此「表达价值上升」这个论点在本项目内**没有正面取证**。取证走另一条路：**选资源的过程本身就是美术决策行为**。这一点在 §4.2 说明。
>
> 文中图表用 Mermaid 绘制，浅色系配色统一：**琥珀**＝判据 / 关键结论　**红**＝危险 / 反例　**绿**＝安全 / 增长　**灰**＝中性过程。
> VS Code 需装 *Markdown Preview Mermaid Support*；标注「示意」的图不含实测数据。
...
`[待补]` 需要一例正面取证：**项目里有没有哪一次，因为「读不出交互」而否掉了更好看的那个资源？** 有这么一例，本节就从推演变成案例。（见 §10 待确认 ①）
...
> 本章的立论方式本身就要求引用可靠。项目规矩：**一条自信的错误注释，比没有注释危害大得多** —— 引用同理。

### 9.1 已核查（可直接引用）

| 引用 | 核查了什么 |
|---|---|
| Doug Church, *Formal Abstract Design Tools*（Game Developer, 1999） | Intention / Perceivable Consequence 两个工具确为原文所列 |
| Salen & Zimmerman, *Rules of Play*（2004） | meaningful play 的两个条件 discernable + integrated 表述准确 |
| Brynjolfsson, Li & Raymond, *Generative AI at Work* | 整体 +14%，新手 +34～35%，熟手增益微弱 |

### 9.2 未核查 —— 落入正式讲稿前必须查

| 项 | 要查什么 |
|---|---|
| Mark Rosewater「restrictions breed creativity」 | 论点已确认存在于其专栏，但**具体篇名与年份未核** |
| 上田文人「减法设计 / design by subtraction」 | 多为访谈转述，**原始出处未定位** |

### 9.3 引用纪律

本章有两处是**我的判断**而不是引用，讲课时必须区分开，否则一旦被追问，整章可信度一起掉：

1. §5.2 无约束的表达无法被识别为表达
2. §4.2 策展是创作的一种形态
...
| # | 问题 | 影响 |
|---|---|---|
| ① | 项目里**有没有因为「读不出交互」而否掉更好看资源**的实例？ | 有 → §4.2 从推演升级为案例，本章最强取证 |
| ② | 否决清单要不要现场产出一份**本项目真实版本**（10 条左右）？ | 有 → §6.1 从方法论变成可带走的东西 |
| ③ | 本章与维度一的顺序：紧接维度一讲，还是放在维度三之后？ | 我倾向紧接 —— 「判据 → 可读」是连续的一条线 |
| ④ | 特殊怪剪影的可读性，要不要做一次**五人盲测**当现场数据？ | 成本低、说服力高，但要提前一天做 |
| ⑤ | 正式生产阶段的资产验收流程要不要纳入本次分享？ | `[待议]` 可能单开一场 |
